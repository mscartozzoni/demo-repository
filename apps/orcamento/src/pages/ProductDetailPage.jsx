          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex flex-col">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="flex items-center border rounded-full p-1">
                <Button onClick={() => handleQuantityChange(-1)} variant="ghost" size="icon" className="rounded-full h-8 w-8 hover:bg-primary/10"><Minus size={16} /></Button>
                <span className="w-10 text-center font-bold">{quantity}</span>
                <Button onClick={() => handleQuantityChange(1)} variant="ghost" size="icon" className="rounded-full h-8 w-8 hover:bg-primary/10"><Plus size={16} /></Button>
              </div>
            </div>
            ...
            <div className="mt-auto">
              ...
              {isStockManaged && canAddToCart && product.purchasable && (
                <p className="text-sm text-green-500 dark:text-green-400 mt-3 flex items-center justify-center gap-2">
                  <CheckCircle size={16} /> {availableStock} em estoque!
                </p>
              )}

              {isStockManaged && !canAddToCart && product.purchasable && (
                 <p className="text-sm text-yellow-500 dark:text-yellow-400 mt-3 flex items-center justify-center gap-2">
                  <XCircle size={16} /> Estoque insuficiente. Apenas {availableStock} restantes.
                </p>
              )}

              {!product.purchasable && (
                  <p className="text-sm text-red-500 dark:text-red-400 mt-3 flex items-center justify-center gap-2">
